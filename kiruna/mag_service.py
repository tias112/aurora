...
class MagWatcher:
    def __init__(self, freq, utc_shift, night_start=16, night_end=3, bz_shift_min=90):
        self.current_ts_utc = datetime.datetime.now() - datetime.timedelta(minutes=40) - datetime.timedelta(
            hours=utc_shift)
        self.current_bz_ts_utc = datetime.datetime.now() - datetime.timedelta(minutes=120) - datetime.timedelta(
            hours=utc_shift)
        print("init current_ts_utc: " + str(self.current_ts_utc), flush=True)
        ...
        print(f"set utc shift: {str(utc_shift)}", flush=True)
    ...
    def calculate_bytes(self):
        ...
        print(f"delay:{str(delta)} ", flush=True)
...
def watcher_service(watcher, irfClient):
    ...
    print(f"current hour {datetime.datetime.utcnow().hour}, is night time: {watcher.is_night_time()}", flush=True)
    t = threading.current_thread()
    n_skip = round(watcher.frequency / 10)
    cnt = 0
    data = ""
    while getattr(t, "do_run", True):
        if cnt % n_skip == 0 and watcher.is_night_time() and not kiruna_is_broken(data, cnt):  # time to run
            ...
            if watcher.x[0] == 0.0 and watcher.y[0] == 0.0 and len(lines) > 0:
                print("init deque first time", flush=True)
...
def bz_watcher_service(watcher, noaaClient):
    ...
    print("stopping Bz watcher", flush=True)
